//
//  LSJSpreadBannerViewController.m
//  LSJVideo
//
...
@interface LSJSpreadBannerViewController () <SDCycleScrollViewDelegate>
{
...
@implementation LSJSpreadBannerViewController
...
//    self.backgroundImage = nil;
    ...
    [_spreads enumerateObjectsUsingBlock:^(LSJProgramModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop)
     {
...
- (void)cycleScrollView:(SDCycleScrollView *)cycleScrollView didSelectItemAtIndex:(NSInteger)index {
    if (index < _spreads.count) {
        LSJProgramModel *spread = _spreads[index];
        if (spread.offUrl.length > 0 && [LSJUtil isVip]) {
