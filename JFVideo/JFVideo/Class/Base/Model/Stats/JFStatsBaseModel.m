...
@interface JFStatsInfoMeta : DBPersistence
...
@implementation JFStatsInfoMeta
@synthesize currentId = _currentId;

+ (instancetype)sharedMeta {
    static JFStatsInfoMeta *_sharedMeta;
...
@implementation JFStatsInfo

//- (instancetype)init {
//    self = [super init];
//    if (self) {
//        _statsId = @([JFStatsInfoMeta sharedMeta].nextId);
//        _appId = JF_REST_APP_ID;
//        _pv = JF_REST_PV.stringValue;
//        _userId = [JFUtil userId];
//        _osv = @"iv";
//    }
//    return self;
//}
...
@implementation JFStatsResponse
...
@implementation JFStatsBaseModel

+ (Class)responseClass {
    return [JFStatsResponse class];
}

- (NSDictionary *)encryptWithParams:(NSDictionary *)params {
//    NSDictionary *signParams = @{  @"appId":JF_REST_APP_ID,
//                                   @"key":kSignKey,
...
- (NSURL *)baseURL {
    return [NSURL URLWithString:JF_STATS_BASE_URL];
}
...
- (NSArray<NSDictionary *> *)validateParamsWithStatsInfos:(NSArray<JFStatsInfo *> *)statsInfos shouldIncludeStatsType:(BOOL)includeStatsType {
    NSArray<JFStatsInfo *> *validStatsInfos = [statsInfos bk_select:^BOOL(JFStatsInfo *statsInfo) {
        if (statsInfo.userId.length == 0) {
            statsInfo.userId = [JFUtil userId];
        }
    ...
    [statsInfos enumerateObjectsUsingBlock:^(JFStatsInfo * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
    ...
    [statsTypes enumerateObjectsUsingBlock:^(NSNumber * _Nonnull obj, BOOL * _Nonnull stop) {
        ...
        [validStatsInfos enumerateObjectsUsingBlock:^(JFStatsInfo * _Nonnull statsInfo, NSUInteger idx, BOOL * _Nonnull stop) {
