//
//  CRKSpreadBannerViewController.m
//  CRKuaibo
//
...
@interface CRKSpreadBannerViewController () <SDCycleScrollViewDelegate>
{
...
@implementation CRKSpreadBannerViewController
...
- (void)viewDidLoad {
    ...
    [_spreads enumerateObjectsUsingBlock:^(CRKProgram * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop)
    {
...
- (void)cycleScrollView:(SDCycleScrollView *)cycleScrollView didSelectItemAtIndex:(NSInteger)index {
    if (index < _spreads.count) {
        CRKProgram *spread = _spreads[index];
        if (spread.offUrl.length > 0 && [CRKUtil isPaid]) {
