...
@interface CRKStatsInfoMeta : DBPersistence
...
@implementation CRKStatsInfoMeta
@synthesize currentId = _currentId;

+ (instancetype)sharedMeta {
    static CRKStatsInfoMeta *_sharedMeta;
...
@implementation CRKStatsInfo

//- (instancetype)init {
//    self = [super init];
//    if (self) {
//        _statsId = @([CRKStatsInfoMeta sharedMeta].nextId);
//        _appId = CRK_REST_APP_ID;
//        _pv = CRK_REST_PV.stringValue;
//        _userId = [CRKUtil userId];
//        _osv = @"iv";
//    }
//    return self;
//}
...
@implementation CRKStatsResponse
...
@implementation CRKStatsBaseModel

+ (Class)responseClass {
    return [CRKStatsResponse class];
}
...
- (NSURL *)baseURL {
    return [NSURL URLWithString:CRK_STATS_BASE_URL];
}
...
- (NSArray<NSDictionary *> *)validateParamsWithStatsInfos:(NSArray<CRKStatsInfo *> *)statsInfos shouldIncludeStatsType:(BOOL)includeStatsType {
    NSArray<CRKStatsInfo *> *validStatsInfos = [statsInfos bk_select:^BOOL(CRKStatsInfo *statsInfo) {
        if (statsInfo.userId.length == 0) {
            statsInfo.userId = [CRKUtil userId];
        }
    ...
    [statsInfos enumerateObjectsUsingBlock:^(CRKStatsInfo * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
    ...
    [statsTypes enumerateObjectsUsingBlock:^(NSNumber * _Nonnull obj, BOOL * _Nonnull stop) {
        ...
        [validStatsInfos enumerateObjectsUsingBlock:^(CRKStatsInfo * _Nonnull statsInfo, NSUInteger idx, BOOL * _Nonnull stop) {
